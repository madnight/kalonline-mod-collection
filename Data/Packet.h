int __fastcall DBProcess(void *Socket, void *edx, int Data)
{
    ...
    if (Type == 1)
    {
        ...
        if (PID)
        {
            ...
            if (Execute((SQLHSTMT*)&dbItem, "SELECT [IID], [ItemStat] FROM Item "
                        "WHERE [PID] = %d", PID))
            {
                while (Fetch(&dbItem))
                {
    ...
    if (Type == 81)
    {
        int PID = 0, Point = 0, Type = 0, HP = 0, HK = 0, HD = 0, DKPT = 0;
        int DKPW = 0, PLT = 0, PLW = 0, SVT = 0, SVW = 0, RP = 0;
        xCDB dbItem;
        CDB(&dbItem);
        Read((char *)pPos, (unsigned int)"ddddddddddd", &PID, &HP, &HK, &HD, &DKPT,
                &DKPW, &PLT, &PLW, &SVT, &SVW, &RP);

        if (PID)
        {
            Execute((SQLHSTMT*)&dbItem, "UPDATE Player SET [HonorPoint] = %d, "
                    "[HonorKill] = %d, [HonorDeath] = %d, [DKPTotal] = %d, "
                    "[DKPWin] = %d, [PLTotal] = %d, [PLWin] = %d, [SVTotal] = %d, "
                    "[SVWin] = %d, [RewardPoint] = %d WHERE [PID] = %d",
                    HP, HK, HD, DKPT, DKPW, PLT, PLW, SVT, SVW, RP, PID);
            ...
            Execute((SQLHSTMT*)&dbItem, "UPDATE Player SET [HonorPoint] = %d, "
                    "[HonorKill] = %d, [HonorDeath] = %d, [DKPTotal] = %d, [DKPWin] = %d, "
                    "[PLTotal] = %d, [PLWin] = %d, [SVTotal] = %d, [SVWin] = %d, "
                    "[RewardPoint] = %d WHERE [PID] = %d",
                    HP, HK, HD, DKPT, DKPW, PLT, PLW, SVT, SVW, RP, PID);
        }
    ...
    if (Type == 82)
    {
        ...
        if (PID)
        {
            ...
            if (Execute((SQLHSTMT*)&dbItem, "SELECT [HonorPoint], [HonorKill], "
                        "[HonorDeath], [DKPTotal], [DKPWin], [PLTotal], [PLWin], "
                        "[SVTotal], [SVWin], [RewardPoint] FROM Player WHERE [PID] = %d", PID))
            {
                Fetch(&dbItem);
                Sleep(5);
                ServerSend((int)Socket, 50, "ddddddddddd", PID, HP, HK, HD, DKPT,
                        DKPW, PLT, PLW, SVT, SVW, RP);
            }
        }
    ...
    if (Type == 83)
    {
        ...
        if (PID)
        {
            Execute((SQLHSTMT*)&dbItem, "UPDATE Player SET [EmokDay] = %d, "
                    "[EmokTime] = %d WHERE [PID] = %d", Day, Time, PID);
    ...
    if (Type == 84)
    {
        ...
        if (PID)
        {
            ...
            if (Execute((SQLHSTMT*)&dbItem, "SELECT [EmokDay], [EmokTime] "
                        "FROM Player WHERE [PID] = %d", PID))
            {
                Fetch(&dbItem);
                Sleep(5);
    ...
    if (Type == 85)
    {
        ...
        if (IID)
        {
            ...
            if (Execute((SQLHSTMT*)&dbItem, "SELECT [PID],[PetTime],[Lock] FROM Item "
                        "WHERE [IID] = %d", IID))
            {
                Fetch(&dbItem);
                Sleep(5);
    ...
    if (Type == 87)
    {
        ...
        if (PID && IID)
        {
            Execute((SQLHSTMT*)&dbItem, "UPDATE Item SET [SetGem] = %d WHERE "
                    "[PID] = %d AND [IID] = %d", SetGem, PID, IID);
            Execute((SQLHSTMT*)&dbItem, "UPDATE Item SET [SetGem] = %d WHERE "
                    "[PID] = %d AND [IID] = %d", SetGem, PID, IID);
            Execute((SQLHSTMT*)&dbItem, "UPDATE Item SET [SetGem] = %d WHERE "
                    "[PID] = %d AND [IID] = %d", SetGem, PID, IID);
    ...
    if (Type == 89)
    {
        ...
        if (PID && IID && PetLife)
        {
            Execute((SQLHSTMT*)&dbItem, "UPDATE Item SET [PetTime] = %d WHERE "
                    "[PID] = %d AND [IID] = %d", PetLife, PID, IID);
            Execute((SQLHSTMT*)&dbItem, "UPDATE Item SET [PetTime] = %d WHERE "
                    "[PID] = %d AND [IID] = %d", PetLife, PID, IID);
            Execute((SQLHSTMT*)&dbItem, "UPDATE Item SET [PetTime] = %d WHERE "
                    "[PID] = %d AND [IID] = %d", PetLife, PID, IID);
    ...
    if (Type == 90)
    {
        ...
        if (PID && IID)
        {
            Execute((SQLHSTMT*)&dbItem, "UPDATE Item SET [ItemStat] = %d WHERE "
                    "[PID] = %d AND [IID] = %d", ItemStat, PID, IID);
            Execute((SQLHSTMT*)&dbItem, "UPDATE Item SET [ItemStat] = %d WHERE "
                    "[PID] = %d AND [IID] = %d", ItemStat, PID, IID);
            Execute((SQLHSTMT*)&dbItem, "UPDATE Item SET [ItemStat] = %d WHERE "
                    "[PID] = %d AND [IID] = %d", ItemStat, PID, IID);
    ...
    if (Type == 91)
    {
        ...
        if (PID && Quest)
        {
            Execute((SQLHSTMT*)&dbItem, "UPDATE Quest SET [QuestTime] = %d, "
                    "[QuestRepeat] = %d, [MonsterCount] = %d WHERE [PID] = %d "
                    "AND [Quest] = %d", Time, Repeat, Count, PID, Quest);
            Execute((SQLHSTMT*)&dbItem, "UPDATE Quest SET [QuestTime] = %d, "
                    "[QuestRepeat] = %d, [MonsterCount] = %d WHERE [PID] = %d "
                    "AND [Quest] = %d", Time, Repeat, Count, PID, Quest);
            Execute((SQLHSTMT*)&dbItem, "UPDATE Quest SET [QuestTime] = %d, "
    ...
    {
        int PID = 0;
        DWORD QuestID = 0, QuestClear = 0, QuestTime = 0, QuestRepeat = 0, MonsterCount = 0;
        xCDB ydbItem;
        CDB(&ydbItem);
        Read((char *)pPos, (unsigned int)"d", &PID);
        ...
        if (PID)
        {
            ...
            if (Execute((SQLHSTMT*)&ydbItem, "SELECT TOP 100 [Quest], [Clear], "
                        "[QuestTime], [QuestRepeat], [MonsterCount] FROM Quest "
                        "WHERE [PID] = %d", PID))
            {
                while (Fetch(&ydbItem))
                {
                    ...
                    {
                        Sleep(5);
                        ServerSend((int)Socket, 56, "dddddd", PID, QuestID, QuestClear,
                                QuestTime, QuestRepeat, MonsterCount);
                    }
                }
            }
        }
