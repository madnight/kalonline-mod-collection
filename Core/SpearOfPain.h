	...
	if (IPlayer.IsValid() && pSkill)
	{
		...
		if (pTarget && nSkillGrade && IPlayer.IsValid())
		{
			...
			if(IPlayer.IsValid() && Target.IsValid())
			{
				...
				int nDmg = ((IPlayer.GetMaxMagAttack() + (((IPlayer.GetMaxMagAttack() / 100) + (CChar::GetInt((int)IPlayer.GetOffset()) / 4)) + ((CChar::GetWis((int)IPlayer.GetOffset()) / 4) + (nSkillGrade * 1)))) * 4) + (IPlayer.GetLevel() * 1);
