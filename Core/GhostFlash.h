	...
	if (IPlayer.IsValid() && pSkill)
	{
		...
		if (pTarget && nSkillGrade && IPlayer.IsValid())
		{
			...
			if(IPlayer.IsValid() && Target.IsValid())
			{
				...
				int nDmg = (IPlayer.GetMaxMagAttack() + ((IPlayer.GetMaxMagAttack() + CChar::GetInt((int)IPlayer.GetOffset())) + (CChar::GetWis((int)IPlayer.GetOffset()) + (nSkillGrade * 60)))) + (IPlayer.GetLevel() * 7);			
