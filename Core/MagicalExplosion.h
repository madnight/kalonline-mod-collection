	...
	if (IPlayer.IsValid() && pSkill)
	{
		...
		if (pTarget && nSkillGrade && IPlayer.IsValid())
		{
			...
			if(IPlayer.IsValid() && Target.IsValid())
			{
				...
				int nDmg = (((IPlayer.GetMaxMagAttack()+ExtraDamage) + ((((IPlayer.GetMaxMagAttack()+ExtraDamage) / 4) + (CChar::GetInt((int)IPlayer.GetOffset()) / 4)) + ((CChar::GetWis((int)IPlayer.GetOffset()) / 4) + (nSkillGrade * 450))))) + (IPlayer.GetLevel() * 2);		
