void __fastcall MAILProcess(void *Player, void *edx, int Type, int pPacket, int pPos)
{
	...
	if (IPlayer.IsOnline() && Type == 7 && !IPlayer.IsBuff(343))
	{
		...
		int Index = 0, Prefix = 0, MaxEnd = 0, CurEnd = 0, Info = 0, Num = 0, XHit = 0, XDefense = 0, XAttack = 0, XMagic = 0, XDodge = 0, Protect = 0, UpgrLevel = 0, SetGem = 0, ItemStat = 0;
		...
		query << "SELECT [Index],[MaxEnd],[CurEnd],[Info],[Num],[XHit],[XDefense],[XAttack],[XMagic],[XDodge],[Protect],[UpgrLevel],[SetGem],[Prefix],[ItemStat] FROM [Item] WHERE [IID] = '" << nIID << "'";
