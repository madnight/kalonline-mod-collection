...
int __fastcall Tick(void *Player, void *edx)
{
	...
	if (IPlayer.IsOnline() && IPlayer.IsBuff(174) && IPlayer.GetBuffRemain(174) <= 5)
	{
	...
	if (IPlayer.IsOnline() && IPlayer.GetClass() == 4 && IPlayer.GetLevel() >= 50 && IPlayer.GetSpecialty() > 3)
	{
		IPlayer.IncreaseMana(MageManaCirculation);
		IPlayer.SetBuffIcon(-2, -1, 2260, 251);
	}
	...
	if (IPlayer.IsOnline() && (GetTickCount() / 1000) % 5 == 0 && !IPlayer.IsBuff(417))
	{
		for (int i = 30; i <= 32; i++)
		{
			...
			if (Buff)
			{
				...
				CPlayer::Write(IPlayer.GetOffset(),68,"bId",74,*(DWORD*)(Buff + 24),0,*(DWORD*)(Buff + 8) - time(0));
			}
		}
		...
		for (int i = 51; i <= 64; i++)
		{
			...
			{
				CPlayer::Write(IPlayer.GetOffset(),68,"bI",75,*(DWORD*)(Buff + 24),0);
				CPlayer::Write(IPlayer.GetOffset(),68,"bId",74,*(DWORD*)(Buff + 24),0,*(DWORD*)(Buff + 8));
			}
		}
