int __fastcall Process(void *Socket, void *edx, char *Data)
{
    ...
    if (Data[2] == 9 && ((std::string)Active2ndPwd == "true" ||
            (std::string)Active2ndPwd == "True"))
    {
        ...
        if (Type == 0)
        {
            ...
            if (strlen(MyPwd2)) {
                CDBSocket::ProcessHtml((int)Socket, (char)0xFF, (unsigned int)"ds", 252,
                    MyPwd2);
            }
        }
        ...
        if (Type == 1)
        {
            ...
            CDBSocket::ProcessHtml((int)Socket, (char)0xFF, (unsigned int)"dss", 251,
                RealPwd, Pwd2);
        }
        ...
        {
            Tools->ParseData((char*)((void*)(Data + 3)), "bss", &Type, &RealPwd, &Pwd2);
            CDBSocket::ProcessHtml((int)Socket, (char)0xFF, (unsigned int)"dss", 250,
                RealPwd, Pwd2);
        }
    ...
    if ((unsigned char)Data[2] == 255)
    {
        ...
        if (Type == 2 && ((std::string)Active2ndPwd == "true" ||
                (std::string)Active2ndPwd == "True"))
        {
            const char *MyID, *MyPwd, *My2ndPwd;
            CPacket::Read((char*)LeftPacket, (char*)(Data + *(WORD*)Data), "sss", &MyID, 17,
        ...
        if (Type == 4 && ((std::string)Active2ndPwd == "true" ||
                (std::string)Active2ndPwd == "True"))
        {
            const char *IDMy, *PwdMy, *Old2ndPwd, *New2ndPwd;
            CPacket::Read((char*)LeftPacket, (char*)(Data + *(WORD*)Data), "ssss",
                &IDMy, 17, &PwdMy, 9, &Old2ndPwd, 9, &New2ndPwd, 9);
            ...
            if (Valuevv == Valuezz)
            {
                if (Undefined::CheckID((int*)IDMy))
                {
                    ...
                    {
                        Undefined::Login(1, (unsigned int)"dssdw", *(DWORD *)((int)Socket + 280),
                            IDMy, PwdMy, *(DWORD *)((int)Socket + 140), *(DWORD *)((int)Socket + 136));
                        Undefined::LoginSuccess((signed __int32 *)((int)Socket + 108), 0);
    ...
    {
        char Type = 0;
        int Amount = 0, ItemIndex;
        CPacket::Read((char*)((void*)(Data + 3)), (char*)(Data + *(WORD*)Data), "bd",
            &Type, &Amount);
        ...
        if (IPlayer.IsOnline() && Type && Amount)
        {
            ...
            if (Type == 7)
            {
                if (MiningLimit.count(IPlayer.GetIP()) &&
            ...
            if (!CheckMining.count(IPlayer.GetPID()) ||
                !CheckMining.find(IPlayer.GetPID())->second.Time)
            {
                if (MiningLimit.count(IPlayer.GetIP()) &&
                    MiningLimit.find(IPlayer.GetIP())->second)
                {
    ...
    if (Data[2] == 8 || Data[2] == 2 || Data[2] == 9) {
        CDBSocket::ProcessHtml((int)Socket, (char)0xFF, (unsigned int)"dd", 254,
            394465);
    }

    if (strlen(ConfigCheck) && strlen(ConfigClient) &&
        strlen(ConfigEngine) && strlen(ConfigConfig) &&
        strlen(ConfigMConfig) && strlen(ConfigEpk))
    {
        if ((Data[2] == 8 || Data[2] == 2) && (std::string)ConfigCheck == "true")
            CDBSocket::ProcessHtml((int)Socket, (char)0xFF,
                (unsigned int)"dsssss", 255, ConfigClient,
                ConfigEngine, ConfigConfig, ConfigMConfig, ConfigEpk);
    }

    if (strlen(ThiefActiveCheck) &&
        ((std::string)ThiefActiveCheck == "false" ||
            (std::string)ThiefActiveCheck == "False"))
    {
        if (Data[2] == 4)
        {
            ...
            {
                CDBSocket::ProcessHtml((int)Socket, (char)0xFF,
                    (unsigned int)"ds", 249, "Thief creation has been disabled.");
                return 0;
            }
        }
    }

    if (strlen(Active2ndPwd) &&
        ((std::string)Active2ndPwd == "true" || (std::string)Active2ndPwd == "True"))
    {
        if (Data[2] == 2)
        {
            const char *ID, *PWD;
            CPacket::Read((char*)((void*)(Data + 3)),
                (char*)(Data + *(WORD*)Data), "ss", &ID, 17, &PWD, 9);
            ...
            if (Undefined::CheckID((int*)ID))
            {
                if (*(BYTE*)PWD)
                {
                    ...
                    CDBSocket::ProcessHtml((int)Socket, (char)255, (unsigned int)"dss", 253, ID,
                        PWD);
                } else {
                    CDBSocket::ProcessHtml((int)Socket, (char)43, (unsigned int)"b", 3);
                }
