	...
	if (IPlayer.IsValid() && pSkill)
	{
		...
		if (pTarget && nSkillGrade && IPlayer.IsValid())
		{
			...
			if(IPlayer.IsValid() && Target.IsValid())
			{
				...
				int nDmg = (IPlayer.GetMaxMagAttack() + (((IPlayer.GetMaxMagAttack() / 9) + (CChar::GetInt((int)IPlayer.GetOffset()) / 9)) + ((CChar::GetWis((int)IPlayer.GetOffset()) / 9) + (nSkillGrade * 300)))) + (IPlayer.GetLevel() * 2);
