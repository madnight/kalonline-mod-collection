	...
	if (IPlayer.IsValid() && pSkill)
	{
		...
		if (pTarget && nSkillGrade && IPlayer.IsValid())
		{
			...
			if(IPlayer.IsValid() && Target.IsValid())
			{
				...
				int nDmg = (IPlayer.GetMaxMagAttack() + (((IPlayer.GetMaxMagAttack() / 1) + (CChar::GetInt((int)IPlayer.GetOffset()) / 1)) + ((CChar::GetWis((int)IPlayer.GetOffset()) / 1) + (nSkillGrade * 350)))) + (IPlayer.GetLevel() * 10);
