int __fastcall LevelUp(void *Player, void *edx)
{
    ...
    if (IPlayer.IsOnline())
    {
        ...
        if (IPlayer.GetLevel() >= 72 && IPlayer.GetLevel() < 76) {
            StatBonus = 2;
        }

        if (IPlayer.GetLevel() >= 76 && IPlayer.GetLevel() < 81) {
            StatBonus = 3;
        }

        if (IPlayer.GetLevel() >= 81 && IPlayer.GetLevel() < 86) {
            StatBonus = 4;
        }

        if (IPlayer.GetLevel() >= 86 && IPlayer.GetLevel() < 91) {
            StatBonus = 5;
        }

        if (IPlayer.GetLevel() >= 91 && IPlayer.GetLevel() < 96) {
            StatBonus = 6;
        }
