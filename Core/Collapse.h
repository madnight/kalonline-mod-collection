	...
	if (IPlayer.IsValid() && pSkill)
	{
		...
		if (pTarget && nSkillGrade && IPlayer.IsValid())
		{
			...
			if(IPlayer.IsValid() && Target.IsValid())
			{
				...
				int nDmg = ((IPlayer.GetMaxMagAttack() + (((IPlayer.GetMaxMagAttack() / 3) + (CChar::GetInt((int)IPlayer.GetOffset()) / 3)) + ((CChar::GetWis((int)IPlayer.GetOffset()) / 2) + (nSkillGrade * 620)))) * 3) + (IPlayer.GetLevel() * 5);
