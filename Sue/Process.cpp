...
void IProcess::Login(void *Socket, int nId, int nUID)
{
	...
	*((unsigned char*)pBuffer + 2) = 0;
	...
	int nCount = 0;
	Server::CDB::Prepare("SELECT TOP %d [PID], [Name], [Class], [Specialty], [Level], [GID], [Strength], [Health], [Intelligence], [Wisdom], [Dexterity], [Face], [Hair] FROM Player WHERE UID = %d ORDER BY [Level]", this->MaxCharCount, nUID);
	if (Server::CDB::Execute(&db, "SELECT TOP %d [PID], [Name], [Class], [Specialty], [Level], [GID], [Strength], [Health], [Intelligence], [Wisdom], [Dexterity], [Face], [Hair] FROM Player WHERE UID = %d ORDER BY [Level]", this->MaxCharCount, nUID))
	{
		char *d;
		while (Server::CDB::Fetch(&db))
		{
			...
			*d = 0;
			...
			Server::CDB::Prepare("SELECT TOP 13 [Index] FROM Item WHERE [PID] = %d AND ( [Info] & %d) = %d", PID, 17, 1);
			if (Server::CDB::Execute(&dbItem, "SELECT TOP 13 [Index] FROM Item WHERE [PID] = %d AND ( [Info] & %d) = %d", PID, 17, 1))
			{
	...
	*((unsigned char *)delBuffer + 2) = 29;
	...
	Server::CDB::Prepare("SELECT TOP 5 PlayerDeleted.PID, Name, [Level], Class, DayLeft = 14-datediff(dd, DeletedTime, getdate()) FROM PlayerDeleted, Player WHERE PlayerDeleted.UID = %d AND PlayerDeleted.PID= Player.PID ORDER BY DayLeft DESC", nUID);
	if (Server::CDB::Execute(&dbDel, "SELECT TOP 5 PlayerDeleted.PID, Name, [Level], Class, DayLeft = 14-datediff(dd, DeletedTime, getdate()) FROM PlayerDeleted, Player WHERE PlayerDeleted.UID = %d AND PlayerDeleted.PID= Player.PID ORDER BY DayLeft DESC", nUID))
	{
