...
void __fastcall Mods::FriendList::SendList(void *pSocket, void *_edx, char* Packet)
{
    ...
    for(unsigned char i=0; i < fCount; i++)
    {
        ...
        offset = Tools->Compile(offset, "dsbdb", id, name, pFriend ? true : false,
                pFriend ? pFriend[0x1c] : 0, 0);
    }
...
void __cdecl Mods::FriendList::SendAdd(void *vPlayer, unsigned char pType,
        char* pFormat, char fType, long id, char* name, char type)
{
    long *pFriend = (long*)Server::CPlayer::FindPlayer(name);
    Server::CPlayer::Write(vPlayer, 0x12, "bdsbdb", 0, id, name,
            pFriend ? true : false, pFriend ? pFriend[0x1c] : 0, 0);
}
