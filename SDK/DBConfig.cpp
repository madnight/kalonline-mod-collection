...
{
    for(std::map<std::string, IDBConfig::_SConfig*>::iterator I = this->_Configs.begin();
            I != this->_Configs.end(); I++)
...
DBSection::DBSection(std::string Name, std::string _Section) : _sValue(""), _Name(Name)
{
    ...
    for(size_t i=0, begin=0; i < _Section.size(); i++)
    {
        ...
        {
            if (__found) {
                Key = _Section.substr(begin, i-begin);
                break;
            }
        }
    }
...
DBSection& DBSection::operator[](const std::string& Section)
{
    DBSection *Ret = NULL;;
    for(size_t i=0; i < this->_SectionList.size(); i++)
        if (this->_SectionList[i]->_Name == Section) {
            Ret = this->_SectionList[i];
            break;
        }
